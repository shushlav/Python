...
logs = np.genfromtxt(log_file, dtype=np.float64, names=True)    #opens the file while savine the first row as names 
print(logs["CALI"])
print(logs)
#print(np.dtype(logs))
print(np.dtype(logs[0]))
'''
# The first line is a header that has all the log names.
header = log_file.readline()
log_names = header.split()
# can also be done using:
#logs_mine = np.genfromtxt(log_file, dtype=float, names=True)opens the file while savine the first row as names 
print('Names:', log_names)
...
logs[logs == -999.25] = np.nan
print(np.shape(logs))
#logs.reshape(1060, 15)
print(logs)
#logs_non = np.where(logs == -999.25, np.nan, logs)
#print(logs_non)
